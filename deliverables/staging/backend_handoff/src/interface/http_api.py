    ...
    class DemoAPIHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            if parsed.path == "/scenarios":
                self._dispatch(lambda: service.scenarios())
                return
            ...
            if parsed.path == "/scenario":
                params = parse_qs(parsed.query)
                scenario_id = _first_value(params.get("scenario_id"))
                if not scenario_id:
                    self._send_error_payload(
                        HTTPStatus.BAD_REQUEST, "scenario_id must not be empty"
                    )
                    return
                self._dispatch(
        ...
        def do_POST(self) -> None:  # noqa: N802
            ...
            if parsed.path == "/reset":
                ...
                return
            if parsed.path == "/demo/family_trip":
                self._dispatch(
                    lambda: service.seed_family_trip_demo(
                        session_id=body.get("session_id")
                    )
                )
                return
            if parsed.path == "/demo/run":
                self._dispatch(
                    lambda: service.run_scenario(
                        scenario_id=body.get("scenario_id", ""),
                        session_id=body.get("session_id"),
                    )
                )
                return
            if parsed.path == "/demo/advance":
                self._dispatch(
                    lambda: service.advance_active_scenario(
                        session_id=body.get("session_id")
                    )
                )
                return
