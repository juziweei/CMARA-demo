...
def test_normalize_extracted_item_accepts_passenger_health_state() -> None:
    item = {
        ...
        "evidence": "我妻子今天不舒服，空调调高一点到 26 度。",
    }

    extracted = _normalize_extracted_item(item)

    assert extracted is not None
    assert extracted.preference == "ac_temperature"
    assert extracted.value == 26
    assert extracted.condition.to_dict() == {
        "type": "passenger_health_state",
        "operator": "==",
        "target": "sick",
